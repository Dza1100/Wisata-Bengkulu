...
export function CardStack<T extends CardStackItem>({
  ...
  // Use type assertion for arithmetic calculations
  const numericCardWidth = typeof cardWidth === 'number' ? cardWidth : 520;
  const cardSpacing = Math.max(10, Math.round(numericCardWidth * (1 - overlap)));
  ...
  const numericCardHeight = typeof cardHeight === 'number' ? cardHeight : 320;
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ height: Math.max(380, numericCardHeight + 80) }}
        tabIndex={0}
        onKeyDown={onKeyDown}
      >
            ...
            {items.map((item, i) => {
              ...
              const dragProps = isActive
                ? {
                    ...
                    onDragEnd: (
                      ...
                      const threshold = Math.min(160, numericCardWidth * 0.22);
              ...
              return (
                <motion.div
                  key={item.id}
                  className={cn(
                    "absolute bottom-0 rounded-2xl border border-[#2c6e8a]/15 overflow-hidden shadow-sm",
  ...
  return (
    <div className="relative h-full w-full">
      ...
      <div className="relative z-10 flex h-full flex-col justify-end p-5">
        <div className="truncate text-xl font-bold text-white mb-1">
          {item.title}
        </div>
        {item.description ? (
          <div className="mt-1 line-clamp-3 text-sm text-white/90">
            {item.description}
          </div>
        ) : null}
      </div>
